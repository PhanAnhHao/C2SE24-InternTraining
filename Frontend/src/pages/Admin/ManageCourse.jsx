...
const ManageCourse = () => {
    ...
    useEffect(() => {
        const res = axios.get("http://localhost:5000/courses")
            .then(res => {
        ...
        <div className="flex">
            <div className="flex-1">
                                ...
                                <tr key={course._id} className={index % 2 === 0 ? "bg-gray-100" : "bg-white"}>
                                    <td className="py-3 px-4">
                                        {editingId === course._id ? (
                                            <input name="idCourse" value={editForm.idCourse}
                                    ...
                                    <td className="py-3 px-4">
                                        {editingId === course._id ? (
                                            <input name="infor" value={editForm.info}
                                                onChange={handleEditChange}
                                                className="border p-1 rounded w-full" />
                                        ) : course.info}
                                    </td>
                                    <td className="py-3 px-4">
                                        {editingId === course._id ? (
                                            <input name="languageID" value={editForm.languageID?.name || ""}
                                                onChange={handleEditChange}
                                                className="border p-1 rounded w-full" />
                                    ...
                                    <td className="py-3 px-4">
                                        {editingId === course._id ? (
                                            <input name="rating" value={editForm.rating}
                                                onChange={handleEditChange}
                                                className="border p-1 rounded w-full" />
                                        ) : course.rating}
                                    </td>
                                        ...
                                        {editingId === course._id ? (
                                            <>
                                                <button className="bg-green-500 text-white px-3 py-1 rounded"
                                                    onClick={handleEditSubmit}>Save</button>
                                            ...
                                            <>
                                                <button className="bg-blue-500 text-white px-3 py-1 rounded"
                                                    onClick={() => handleEditClick(course)}>Edit</button>
                ...
                <div className="flex justify-center mt-4">
                    <button className={`px-4 py-2 mx-1 rounded ${currentPage === 1 ? "bg-gray-300" : "bg-[#4FD1C5] text-white"}`}
                        onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))}
                        disabled={currentPage === 1}>Prev</button>
                    <span className="px-4 py-2 bg-gray-200 rounded">
                        Page {currentPage} of {totalPages}
                    </span>
                    <button className={`px-4 py-2 mx-1 rounded ${currentPage === totalPages ? "bg-gray-300" : "bg-[#4FD1C5] text-white"}`}
